<!-- <template>
  <div>
    <button @click="convertGeoJsonToShapefile">
...
export default {
  methods: {
    async convertGeoJsonToShapefile() {
      ////// *****  @mapbox/shp-write ******/////////////
      const options = {
...
<!-- <template>
  ...
  <button @click="downloadShapefile">Download Shapefile</button>
</template>
...
export default {
  ...
  methods: {
    initMap() {
      ...
      // Set event listener to capture polyline draw
    ...
    handleDrawCreate(event) {
      ...
      if (features.length > 0) {
        this.polyline = features[0].geometry.coordinates; // Capture polyline coordinates
      }
    },
        ...
        features: [
          {
            ...
            properties: {
              WSITENAME: formData.value.WSITENAME || "",
              WORKSITEID: formData.value.WORKSITEID || "",
              STREETNAME: formData.value.STREETNAME || "",
              STREETW: formData.value.STREETW || "",
              PATHWIDTH: formData.value.PATHWIDTH || "",
              PATHLENGTH: formData.value.PATHLENGTH || "",
              PATHDEPTH: formData.value.PATHDEPTH || "",
              ROADSCRAP: formData.value.ROADSCRAP || "",
              PATHINDEX: formData.value.PATHINDEX || "",
            },
          },
        ],
      };
...
.maplibregl-control-container .maplibregl-ctrl-group.maplibregl-ctrl {
  gap: 10px;
  display: flex;
}
</style> -->
